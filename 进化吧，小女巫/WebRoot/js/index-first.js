...
function Ship(ctx) {
    ...
    this.paint = function () {
        ...
        ctx.color = "black";
        ctx.font = "32px Aria";
        ctx.fillText(this.top, this.left + 75, this.top);
        ...
        ctx.fillText(this.top + this.height, this.left + 75, this.top + this.height + 20);
        ctx.restore();
    };
    ...
    this.eat = function (foodlist) {
        for (var i = foodlist.length - 1; i >= 0; i--) {
            ...
            if (f) {
                var t=this.top-16;
                var l=this.left-16;
                var tl=this.left+this.width-15;
                var bl=this.top+this.height-18;
                if (t< f.top + f.height && tl > f.left && l < f.left + f.width && bl > f.top) {
                    foodlist[f.id] = null;
                    if (f.type != 0) {
                        gameMonitor.stop();
                        //gameMonitor.getScore();
                        if (f.type == 1) {
                            $("#fenghao").addClass("out_gg");
                        }
                        if (f.type == 2) {
                            $("#fenghao").addClass("out_kl");
                        }
                        if (f.type == 3) {
                            $("#fenghao").addClass("out_ss");
                        }
                        if (f.type == 4) {
                            $("#fenghao").addClass("out_bf");
                        }
                        $('#resultPanel').show();
                    } else {
...
Food.prototype.paint = function (ctx) {
    ...
    ctx.fillText(parseInt(this.left + this.width), this.left + this.width, this.top + this.height / 2);
    ctx.fillText(parseInt(this.left), this.left - 60, this.top + this.height / 2);
    ctx.fillText(parseInt(this.top + this.height), this.left + 75, this.top + this.height + 20);
    ctx.restore();
};
