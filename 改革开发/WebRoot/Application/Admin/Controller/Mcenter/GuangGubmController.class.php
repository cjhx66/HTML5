...
//导出excel
	public function excel(){
		...
		header("Content-Disposition: attachment;filename=".'改革开放留言'.".xls");
		header("Content-Transfer-Encoding: binary ");
		$model = M("guanggu");

		
		$result = $model->order('writetime desc')->select();
		...
		foreach ($result as $k => &$v) {
			$data[$k]['id'] = $k+1;
			$data[$k]['name'] = $v['name'];
			$data[$k]['writetime'] = date("Y-m-d H:i:s",$v['writetime']);
		}
		$str.='<table border="1" width="1000">';
		$str.='<tr>
				<th>编号</th><th>留言</th><th>留言日期</th>
			  </tr>';
